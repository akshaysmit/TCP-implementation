...
enum status_code {SS, CA, FR}; //FR = fast recovery
unsigned CA_counter = 0;
int FR_trigger_index;     //packet that caused fast recovery/retransmission
status_code status = SS;
int not_found_error = 0;
int portno, sockfd;
socklen_t clilen;
struct sockaddr_in serv_addr, cli_addr;
long min_index = 0;
long max_index = 1;       //window size starts at 1
uint64_t send_base;
...
void move_window(int ind) { 
  unsigned gap = max_index - min_index;
  
  if (ack_status[ind] >= 4 && status != FR) { //3 duplicate ACKs
    SSTHRESH = gap * (MAX_PACKET_SIZE / 2);
    if (SSTHRESH <= 2*MAX_PACKET_SIZE)
      SSTHRESH = 2*MAX_PACKET_SIZE;

    min_index = ind+1;
    max_index = min_index + SSTHRESH/MAX_PACKET_SIZE + 3;
    if ((max_index - min_index) * MAX_PACKET_SIZE >= rwnd) //don't let window get too big
      max_index= min_index + rwnd/MAX_PACKET_SIZE;
    
    status = FR;
    FR_trigger_index = ind+1;
    send_data(ind+1);                //fast retransmission
    print_send(headers[ind+1].seq_n, (max_index - min_index) * MAX_PACKET_SIZE, 1, 0, 0);
  }
  else if (status == FR) { 
  ...
  else if (status == SS && ack_status[ind] == 1) {
    max_index = (ind+1) + gap + (ind+1 - min_index);  //window increases by 1 MSS upon ACK
    min_index = ind+1;
    if ((max_index - min_index) * MAX_PACKET_SIZE >= SSTHRESH)
      status = CA;
  }
  else if (status == CA && ack_status[ind] == 1) {
    CA_counter += (ind + 1 - min_index);
    min_index = ind+1;
    max_index = min_index + gap;
...
void *ACKreader(void *arg) {  //this thread handles reception of ACK's
  while (1) {
    ...
    int j;
    for (j = min_index - 1; j < num_packets; j++) {
      if (j < 0)
	continue;
    ...
    if (j >= num_packets) {  //No corresponding packet
    ...
    ack_status[j] += 1;     //increment ACK count
    move_window(j);
...
void handshake(int fd, struct sockaddr_in *addr, socklen_t *addr_len) {
  ...
  while(1) {
    ...
    if(send_hdr(&pkt) == -1)
      exit(1);
    print_send(pkt.seq_n, (max_index - min_index) * MAX_PACKET_SIZE, retrans, 1, 0);
    pthread_mutex_unlock(&master_lock);
...
void create_headers() {
  ...
  for (int i = 0; i < num_packets; i++) {
    timer_active[i] = 0;
    ack_status[i] = 0;   //this means not acked 
  }
...
void send_file() {
  ...
  while (1) {
	...
	if (send_data(i) == -1)
	  exit(1);
	print_send(headers[i].seq_n, (max_index - min_index) * MAX_PACKET_SIZE, 0, 0, 0);
	timer_active[i] = 1;
      }
      
      else if (i == min) {            //only poll timer for min
	std::chrono::time_point<std::chrono::steady_clock> end = std::chrono::steady_clock::now();
	double t = std::chrono::duration_cast<std::chrono::milliseconds>(end - timers[i]).count();
	if (t < TIMEOUT) 
	  continue;
	
	status = SS;                     //timeout occured, change window
	SSTHRESH = (max_index - min_index) * (MAX_PACKET_SIZE / 2);
	if (SSTHRESH <= 2*MAX_PACKET_SIZE)
	  SSTHRESH = 2*MAX_PACKET_SIZE;
	if (min_index > 0)
	  ack_status[min_index - 1] = 1; //forget any duplicate ACK's
	
	max_index = min_index + 1;
	timers[i] = std::chrono::steady_clock::now(); //reset clock
	if (send_data(i) == -1)
	  exit(1);
	print_send(headers[i].seq_n, (max_index - min_index) * MAX_PACKET_SIZE, 1, 0, 0);
	break;
      }
    }
...
void fin() {
  ...
  while(1) {
    ...
    if(send_hdr(&pkt) == -1)
      exit(1);
    count++;
    print_send(pkt.seq_n, (max_index - min_index) * MAX_PACKET_SIZE, retrans, 0, 1);
    pthread_mutex_unlock(&master_lock);

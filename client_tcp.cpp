...
const int BUFFER_NPKTS = 50;
size_t buffered_sizes[BUFFER_NPKTS];
int spot_taken[BUFFER_NPKTS];
struct packet_hdr buffered_hdr[BUFFER_NPKTS];
char BUFFER[BUFFER_NPKTS][MAX_PACKET_SIZE];
...
void move_window() {
  for (int i = 0; i < BUFFER_NPKTS; i++) {
    for (int j = 0; j < BUFFER_NPKTS; j++) {
      if (spot_taken[j] == 1 && buffered_hdr[j].seq_n == rcv_base) {
	spot_taken[j] = 0;
	rcv_base = buffered_hdr[j].seq_n + buffered_sizes[j] + sizeof(struct packet_hdr);
...
void get_file() {
  while(1) {
    ...
    else if (pkt.seq_n < rcv_base) { //must send ACK to move sender window
      pkt.ack_flag = 1;
      pkt.fin_flag = 0;
      pkt.ack_n = rcv_base;          //ACK last received in-order packet
      send_hdr(&pkt);
	...
	buffered_hdr[ind] = pkt;
      ...
      pkt.ack_flag = 1;
      pkt.fin_flag = 0;
      rcv_base = pkt.seq_n + bytes + sizeof(struct packet_hdr);
      move_window();       //move rcv_base based on buffered packets
      pkt.ack_n = rcv_base;
      send_hdr(&pkt);
      print_send(pkt.ack_n, 0, 0, 0);
...
void handshake() {       //Requires sockfd, servlen, and serv_addr to be initialized
  ...
  while(1) {
    ...
    memset(&pkt, 0, sizeof(struct packet_hdr));
    pkt.rcv_window = MAX_PACKET_SIZE * BUFFER_NPKTS;
    pkt.syn_flag = 1;
